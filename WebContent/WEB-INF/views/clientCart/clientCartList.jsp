	...
	//삭제(장바구니비우기, 개별삭제, 선택삭제)------------------------------------------
  	...
  	$(function(){
  		...
  		$("input[name='allCheck']").click(function(){
  			var sumtprice = "<c:out value='${sumPrice}' />";
  			var sumtmprice = "<c:out value='${sumPrice * 0.8}' />";
  			var sumtmprice1 = parseInt(sumtmprice);
  			var sumtmprice2 = String(sumtmprice1);
  			var sumtotalprice = sumtprice.replace(/\B(?=(\d{3})+(?!\d))/g, ",") +"원"
  			var sumtotalmprice = sumtmprice2.replace(/\B(?=(\d{3})+(?!\d))/g, ",") +"원"
  			...
  			if(mcodeD == 'D'){ 
  				if($("input[name='RowCheck']:checked").length == 0){
  				  				...
  				  				document.getElementById('sumprice').innerHTML="<h2>전체 상품 금액 : " + sumtotalmprice;
  				  			}
  				} else {
  				if($("input[name='RowCheck']:checked").length == 0){
  				  				document.getElementById('sumprice').innerHTML="<h2>선택 상품 금액 : 0원";
  				  			} else {
  				  				document.getElementById('sumprice').innerHTML="<h2>전체 상품 금액 : " + sumtotalprice;
  				  			}
  				}
  		});
...
<section class="ftco-section">
					...
					<div class="table-wrap">
						<table class="table table--block carttable">
						  ...
						  <tbody style="height:12rem;">
						    ...
						    <c:choose>
		<c:when test="${sessionMember.mcode=='D' && c.pcount<30 }">
			<td><input type="hidden" id="cno" name="cno" value="${c.cno }" /></td>
			<td>
				<div id="cartimg">
					<a href="${pageContext.request.contextPath}/pro/detail/${c.pno}"><img src="<spring:url value='/resources/img/${c.img1}'/>" alt="${c.img1}" title="${c.img1 }입니다"/></a>	
				</div>			
			</td>
			<td>
				<a href="${pageContext.request.contextPath}/pro/detail/${c.pno}">${c.pname }</a>
			</td>
			<td>
			<input style="border:none; font-size:14px;" type="text" id="price" value="<fmt:formatNumber value="${c.price}" pattern="#,###,###원"/>" />
			</td>
			<td colspan="2"><span style="color:red; font-size:12px;">구매할 수 없습니다. 다시 담아주세요.</span></td>
			<td><a class="btn btn-outline-danger" href="javascript:deleteCart(${c.cno})">삭제</a></td>
		</c:when>
		<c:otherwise>
						    	...
						    	<td>
						    		<input type="checkbox" name="RowCheck" class="chBox" value="${c.cno }" checked />
									<input type="hidden" id="cno" name="cno" value="${c.cno }" />
						    	</td>
						    	<td>
						    		<div class="img">
						      ...
						      <td>
					        	<c:choose>
					  				<c:when test="${mem.mcode == 'D'}">
					  					<input type="button" class="count_btn" onclick="minus2(this)" value="-">
					  					<input style="width:40px;" type="number" name= "pcount" id="pcount" value="${c.pcount }" min="30" max="100" step="10" readonly required/>
					  					<input type="button" class="count_btn" onclick="plus2(this)" value="+">
					  					<button style="background-color:#365902; padding:7px 10px; height:25px; line-height:1px;" type="submit" class="btn btn-main" style="height:25px;">변경</button>
					  				</c:when>
					  				<c:otherwise>
				         	 ...
				         	 <td>
								<c:choose>
				  				<c:when test="${mem.mcode == 'D'}">	
				  					<span style="color:red; font-size:12px;">(20% 할인)</span><input style="border:none; font-size:14px;" type="text" id="tprice" value="<fmt:formatNumber value="${(c.price * c.pcount) * 0.8}" pattern="#,###,###원"/>" />
				  				</c:when>
				  				<c:otherwise>
				  					<input style="border:none;" type="text" id="tprice" value="<fmt:formatNumber value="${c.price * c.pcount}" pattern="#,###,###원"/>" />
				  				</c:otherwise>
				  				</c:choose>
							</td>
						      <td>
						      	<a class="btn btn-outline-danger" href="javascript:deleteCart(${c.cno})"><i class="fa fa-close"></i></a>
				        	</td>
	...
	<div class="container" style="margin-bottom:20px; width:100%; height:120px;">
				  <c:choose>
				  		<c:when test="${mem.mcode == 'D'}">	
				  			<div id="sumprice"><h2>전체 상품 금액 : <fmt:formatNumber value="${sumPrice*0.8 }" pattern="#,###,###원"/></h2></div>
				  		</c:when>
				  		<c:otherwise>
				  			<div id="sumprice"><h2>전체 상품 금액 : <fmt:formatNumber value="${sumPrice }" pattern="#,###,###원"/></h2></div>
				  		</c:otherwise>
				  </c:choose>

				<div id="orderBtn" style="margin-top:20px;">
